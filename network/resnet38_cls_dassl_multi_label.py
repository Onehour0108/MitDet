...
import network.resnet38d_dassl


class Net(network.resnet38d_dassl.Net):
    def __init__(self, from_round_nb):
        super().__init__()
        ...
        # class 2 + class 8
            ...
            self.fc8_20 = nn.Conv2d(4096, 2, 1, bias=False)
            self.fc8_200 = nn.Conv2d(4096, 8, 1, bias=False)
    ...
    def forward_cam(self, x):
        x = super().forward(x)
        x = F.conv2d(x, self.fc8_20.weight)
        x = F.relu(x)
...
class Net_CAM(network.resnet38d_dassl.Net):
    def __init__(self, from_round_nb):
        super().__init__()
        self.from_round_nb = from_round_nb
    ...
    def forward_output(self, x, enable_PDA):
